...
function arc4(msg, key) {
  ...
  for (i = 0; i < msg.length; i++) {
    ...
    output += (
      '0' +
      (msg[i].charCodeAt(0) ^ state[(state[x] + state[y]) % 256]).toString(16)
    ).slice(-2);
  }
...
function codigoControl(auth, number, nit, date, total, key) {
  code = '';
  ...
  vf = verhoeff(
    '' + (parseInt(number) + parseInt(nit) + parseInt(date) + parseInt(total)),
    5
  ).slice(-5);
  ...
  for (i = 0; i < 5; i++) {
    final_sum += Math.floor(
      (total_sum * partial_sum[i]) / (1 + parseInt(vf[i]))
    );
  }

  code = arc4(base64(final_sum), key + vf)
    .match(/.{2}/g)
    .join('-');
