...
export const generarPlanillasLiquidacion = (datosPlanillas) => {
  const config = {
    ancho: 80,
    alto: 279.4,
    margenDerecho: 4,
...
const dataCabecera = (doc, config, inicioDerecha, puntoMedio) => {
  ...
  const dataCabecera = [
    ...
    (doc) => {
      config.y += 4;
      doc.setFontSize(8);
      doc.text('Empresa de Transporte de Pasajeros', inicioDerecha, config.y, {
        align: 'right',
      });
    },
  ];
  ...
  const docData = [
    ...
    (doc) => {
      ...
      doc
        ...
        .text(`Flota "${nombreEmpresa}"`.toUpperCase(), puntoMedio, config.y, {
          align: 'center',
        });
    },
    (doc) => {
    ...
    (doc) => {
      ...
      doc
        .setFont('helvetica', 'bold')
        .text('Código:', margenIzquierdo, config.y);
    ...
    (doc) => {
      ...
      doc
        .setFont('helvetica', 'bold')
        .text('Origen:', margenIzquierdo, config.y);
      doc
        .setFont('helvetica', 'normal')
        .text(origen.toUpperCase(), 16, config.y);
      doc.setFont('helvetica', 'bold').text('Destino:', 40, config.y);
      doc
        .setFont('helvetica', 'normal')
        .text(destino.toUpperCase(), 53, config.y);
    },
    (doc) => {
      config.y += 4;
      doc
        .setLineWidth(0.2)
        .line(margenIzquierdo, config.y, inicioDerecha, config.y);
    },
    (doc) => {
      /**
  ...
  const docData = [
    ...
    (doc) => {
      pasajes.forEach((pasaje, indice) => {
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('Sucursal:', puntoMedio, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.branchNumber, puntoMedio + margen + 2, config.y);

        config.y += 4;
        doc.setFont('helvetica', 'bold').text('Ticket:', puntoMedio, config.y);
        imprimirCadenaLarga(
          doc,
          config,
          pasaje.ticketNumber,
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('Emitido por:', puntoMedio, config.y);
        imprimirCadenaLarga(
          doc,
          config,
          pasaje.issuingUser,
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('Teléfono:', puntoMedio, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.branchPhone, puntoMedio + margen + 2, config.y);
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('Fecha:', margenIzquierdo, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.travelDate, margen + 2, config.y);

        doc.setFont('helvetica', 'bold').text('Asiento:', puntoMedio, config.y);

        config.y += 4;
        doc
          .setFont('helvetica', 'bold')
          .text('Hora:', margenIzquierdo, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.departureTime, margen + 2, config.y);
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('Carril:', margenIzquierdo, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.lane, margen + 2, config.y);

        config.y += 4;
        doc
          .setFont('helvetica', 'bold')
          .text('Pasajero:', margenIzquierdo, config.y);
        imprimirCadenaLarga(doc, config, pasaje.passengerFullName, margen, 15);

        doc
          .setFont('helvetica', 'bold')
          .text(pasaje.typeOfSeat, puntoMedio + 20, config.y, {
            align: 'center',
          });
        ...
        doc
          .setFont('helvetica', 'bold')
          .text('C.I.:', margenIzquierdo, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(pasaje.identificationNumber, margen + 2, config.y);

        doc.setFont('helvetica', 'bold').text('Precio:', puntoMedio, config.y);
        doc
          .setFont('helvetica', 'normal')
          .text(
        ...
        doc
          .setFont('helvetica', 'bold')
          .text(pasaje.legend, puntoMedio, config.y, { align: 'center' });
...
export const generarManifiestoPasajeros = (datosManifiesto) => {
  ...
  const docData = [
    ...
    (doc) => {
      ...
      doc
        .setFont('helvetica', 'normal')
        .text(`CÓDIGO DE VIAJE: ${tripMadeKey}`, margenIzquierdo, config.y);
    },
    (doc) => {
      config.y += 4;
    ...
    (doc) => {
      config.y += 4;
      doc.text(`FECHA DE SALIDA: ${travelDate}`, margenIzquierdo, config.y);
    },
    (doc) => {
      config.y += 4;
      doc.text(`HORA DE SALIDA: ${departureTime}`, margenIzquierdo, config.y);
    },
    (doc) => {
      config.y += 4;
      doc.text(
...
export const generarListaUsuarios = (datosUsuarios) => {
  ...
  if (usersData.length === 0) {
    ...
    const docData = [
      ...
      (doc) => {
        config.y += 4;
        doc.text(`FECHA DE IMPRESIÓN: ${printDate}`, margenIzquierdo, config.y);
      },
      (doc) => {
        config.y += 4;
        autoTable(doc, {
